...
const ExpenseContent: React.FC = () => {
  ...
  const filteredExpenses = expenses.filter((expense) => {
    const matchesCategory =
      ...
      expense.categoryId === Number(selectedCategory);
  ...
  const handleUpdateExpense = async (id: number) => {
    try {
      ...
      if (editingExpenseData.startDate)
        formData.append("startDate", editingExpenseData.startDate);
      if (editingExpenseData.endDate)
        formData.append("endDate", editingExpenseData.endDate);
      if (receiptFile) formData.append("receipt", receiptFile);
                      ...
                      <>
                        ...
                        <td className="px-6 py-4">
                          <input
                            ...
                            className="border border-[var(--color-secondary-light)] rounded p-1 w-full bg-[var(--color-bg)] text-[var(--color-text)]"
                          />
                        </td>
                        <td className="px-6 py-4">
                          <select
                            ...
                            onChange={(e) =>
                              setEditingExpenseData((prev) => ({
                                ...prev,
                                type: e.target.value as "OneTime" | "Recurring",
                              }))
                            }
                            className="border border-[var(--color-secondary-light)] rounded p-1 w-full bg-[var(--color-bg)] text-[var(--color-text)]"
                          >
                        ...
                        <td className="px-6 py-4 flex gap-2">
                          <button
                            ...
                            className="px-2 py-1 bg-[var(--color-primary)] text-white text-sm rounded-lg hover:brightness-90"
                          >
                            Save
                          </button>
                          <button
                            onClick={() => setEditingExpenseId(null)}
                            className="px-2 py-1 bg-red-600 rounded-lg text-sm hover:opacity-90"
                          >
                            <X className="w-4 h-4" />
                          </button>
                        </td>
                          ...
                          {deleteConfirmId === expense.id ? (
                            <>
                              ...
                              <button
                                ...
                                className="px-2 py-1 bg-gray-300 rounded hover:opacity-90"
                              >
                                <X className="w-4 h-4" />
                              </button>
                            </>
                            ...
                            <>
                              {expense.receipt && (
                                <button
                                  onClick={() =>
                                    handleDownloadReceipt(expense.id)
                                  }
                                  className="flex items-center gap-2 px-3 py-1 bg-[var(--color-secondary)] text-white rounded-lg hover:opacity-90 text-sm"
                                >
                              ...
                              <button
                                ...
                                className="p-1 bg-[var(--color-primary)] text-white rounded hover:opacity-90"
                              >
                                <Edit2 className="w-4 h-4" />
                              </button>
                              <button
                                onClick={() => setDeleteConfirmId(expense.id)}
                                className="p-1 bg-[var(--color-expense)] text-white rounded hover:opacity-90"
                              >
          ...
          <div className="bg-[var(--color-bg-card)] rounded-xl p-6 w-full max-w-md shadow-lg border border-gray-300">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <select
                ...
                value={type}
                onChange={(e) =>
                  setType(e.target.value as "OneTime" | "Recurring")
                }
                className="w-full p-2 border border-gray-300 rounded-xl bg-[var(--color-bg)] text-[var(--color-text)]"
              >
                <option value="OneTime">One Time</option>
              ...
              {type === "Recurring" && (
                <>
                  <div>
                    <label className="block text-sm text-[var(--color-text-sub)] font-medium">
                      Start Date
                    </label>
                    <input
                      type="date"
